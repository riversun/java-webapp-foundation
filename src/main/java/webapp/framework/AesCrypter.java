...
class AesCrypter {
	...
	public String encrypt(String text) {
		...
		try {
			...
			final SecretKeySpec key = new SecretKeySpec(AesCrypterDef.CRYPT_KEY.getBytes(STR_ENCODING), CRYPT_ALGORITHM);
			final IvParameterSpec iv = new IvParameterSpec(AesCrypterDef.CRYPT_IV.getBytes(STR_ENCODING));

			final Cipher cipher = Cipher.getInstance(PADDING);

			cipher.init(Cipher.ENCRYPT_MODE, key, iv);

			final byte[] encrypted = cipher.doFinal(text.getBytes(STR_ENCODING));

			retVal = HexCodec.encodeHexString(encrypted);
	...
	public String decrypt(String text) {
		...
		try {
			...
			final SecretKeySpec secretKeySpec = new SecretKeySpec(AesCrypterDef.CRYPT_KEY.getBytes(STR_ENCODING), CRYPT_ALGORITHM);
			final IvParameterSpec ivParameterSpec = new IvParameterSpec(AesCrypterDef.CRYPT_IV.getBytes(STR_ENCODING));

			final Cipher cipher = Cipher.getInstance(PADDING);

			cipher.init(Cipher.DECRYPT_MODE, secretKeySpec, ivParameterSpec);

			final byte[] decrypted = cipher.doFinal(HexCodec.decodeHex(text.toCharArray()));

			retVal = new String(decrypted, STR_ENCODING);
	...
	/**
	 * Converts hexadecimal Strings. The charset used for certain operation can
	 * be set, the default is set in {@link #DEFAULT_CHARSET_NAME}
	 * 
	 * @since 1.1
	...
	static class HexCodec {
		 ...
		 * @since 1.4
		 */
		public HexCodec(String csName) {
			this.charsetName = csName;
		}

		/**
		 * Converts an array of character bytes representing hexadecimal values
		 * into an array of bytes of those same values. The returned array will
		 * be half the length of the passed array, as it takes two characters to
		 * represent any given byte. An exception is thrown if the passed char
		 * array has an odd number of elements.
		 * 
		 * @param array
		 ...
		 * hexadecimal values into an array of bytes of those same values. The
		 * returned array will be half the length of the passed String or array,
		 * as it takes two characters to represent any given byte. An exception
		 ...
		 * returned array will be double the length of the passed String or
